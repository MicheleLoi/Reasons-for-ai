...
The form of externalism in play here is the standard semantic externalism of Putnam (1975), Burge (1979, 1993), and Kripke (1980). Content-directedness on this view is fixed by causal-historical chains and the social division of linguistic labour, not by the speaker's individual reliability. This is what allows the externalist intuition to be preserved without backsliding into the truth-tracking claim: the screening system can be confirmation-biased and individually unreliable in the way Mercier and Sperber describe, while the *content* of what it produces remains world-directed in virtue of meta-semantic facts that obtain at the level of the linguistic community.

> *Note on the deflationary tension — dissolved. An earlier version of this preservation claimed the screening system "tracks features of situations — others' pain, fairness, consistency." That was too strong: M&S's wider account is partly deflationary (reasoning evolved for argumentative persuasion, is confirmation-biased, does not paradigmatically track truth in the individual case), so the truth-tracking version of the externalist preservation cannot be sourced from M&S and was in tension with their deflationary side.*
>
> *The current formulation weakens the claim: only the* intentional content *of justifications is world-directed, and the semantic-externalist tradition supplies the meta-semantic story (content fixed via causal-historical chains and a linguistic community, not via individual reliability). The tension was a tension only on the truth-tracking formulation; once truth-tracking is dropped, the tension goes with it. The two moves are orthogonal: externalism handles the* aboutness *question (how is content world-directed?); M&S handles the* cognitive-operation *question (what kind of system generates and evaluates these contents, and when is its screening operation upstream of behavior?). The criterion in (i)–(iii) above is M&S's contribution, not externalism's; without M&S there is no criterion at all.*
>
> *What remains, as a check rather than a substantive worry: confirm that the offloading is principled, not evasive — i.e., that we are not smuggling in via Putnam what we wanted to source from M&S. The narrower question for Mercier's judgment: is the production-and-screening architecture, on his own account of it, the right kind of cognitive operation to ground the reasons-responsive / rationalization distinction in §5a — and in particular, can the screening operation be causally upstream of action-generation in the way the criterion requires?*
...
> *Note — load-bearing, to discuss. The argument here is structurally parallel to §3 but meaningfully weaker. §3 leans on phenomenology — the deliberating subject's first-person experience of attending to the world. The LLM case has only output structure, and output structure is precisely what next-token prediction trained on human justificatory discourse is optimized to produce regardless of whether anything internal corresponds to world-directedness. The §6.5b externalist meta-semantics is what the paper offers in response: world-directedness of content does not have to be read off output structure if it can be grounded in the causal-historical chains running through the training corpus. On this construal the response to the mimicry concern is symmetric with §5b — for both humans and LLMs, content-directedness is meta-semantically fixed, not phenomenologically read. The asymmetry between the §3 phenomenological argument and the §6.3 output-structure argument is genuine; the externalist response makes it less load-bearing than it first appears.*
...
The externalist preservation is not a free analogical extension from §5b. The same move has been made directly for LLMs by a small but growing literature on LLM meta-semantics: Cappelen & Dever (2021, 2025), Mandelkern & Linzen (2023), Mollo & Millière (2023). The core argument: an LLM is trained on a corpus produced by speakers already in causal-historical contact with the world; training places the model in mediated causal-historical contact with the same referents; the content of its outputs inherits world-directedness from the corpus. As Cappelen & Dever (2025, ch. 7) put it, "the exposure to this corpus, which again is connected to the world and to other speakers, [is what] connects ChatGPT and its language to the rest of the world." This is the meta-semantic story that grounds (i)–(ii) above on the externalist side, and it is the direct reply to Bender & Koller (2020) — that LLMs manipulate form without semantics — which it answers without conceding that "picking out a referent when presented with physical alternatives" is the relevant standard for content-bearing.

We adopt this externalist meta-semantics as a precedent move; the present paper is not adding to that literature. What the present paper does add is the linkage to Mercier and Sperber's architecture: externalist meta-semantics tells us that LLM outputs are *about* features of the world; M&S tell us *what kind of cognitive operation* produces those outputs (production-and-screening); the symmetric criterion in (i)–(ii) brings these together.
